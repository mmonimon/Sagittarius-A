...
During the execution of this project the provided server with an NVIDIA GeForce GTX 1080Ti broke down.
...
Furthermore, I used the datasets provided on Moodle for development which correspond to plain text versions of the news dev sets for writing a bash script that helped me automating the model building process and to automate the pre-processing and testing procedures. The final version of the script can be found in the Git repository.
...
I starting using BPE Splitting early on for some of the models in order to see if it will improve the BLEU score gradually. For the first model, I did not use BPE conversion, and introduced it for the second one, using 3000 BPE merge operations which did not show a significant improvement. I changed the value to 32000 which almost doubled the BLEU score (2.86 => 5.34) and then, later on increased it to 64000 which again showed an improvement of the BLEU Score (5.79 => 8.35). However, in the course of the project, I noticed that the default vocabulary size (src\_vocab\_size and tgt\_vocab\_size) of onmt\_translate was only 50000. I therefore decided to train the best 64000 BPE model (titan) again with identical parameters but with 40000 BPE merge operations, hoping that this would lead to an improvement in the BLEU score. The name of the adapted model is callisto. The assumption was confirmed and callisto achieved a BLEU score 1.98 points higher than titan.
...
\begin{itemize}
\item Training with more than 100000 steps (--train\_step\_size).
